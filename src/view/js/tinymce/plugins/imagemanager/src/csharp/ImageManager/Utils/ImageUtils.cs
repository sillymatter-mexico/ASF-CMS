/*
 * $Id: ImageUtils.cs 774 2010-03-02 13:28:24Z spocke $
 *
			...
			foreach (string chunk in format.Split(new char[] {','})) {
				...
				if (chunkMatch.Success) {
							...
							default:
								...
								if (match.Success) {
									...
									if (match.Groups[1].Value == "*") {
										// Width is omitted
										targetWidth = (int) Math.Floor(info.Width / (info.Height / targetHeight));
									}

									if (match.Groups[2].Value == "*") {
										// Height is omitted
										targetHeight = (int) Math.Floor(info.Height / (info.Width / targetWidth));
									}
								}
					...
					if (targetWidth != info.Width || targetHeight != info.Height) {
						...
						newWidth = scale > 1 ? info.Width : (int) Math.Floor(info.Width * scale);
						newHeight = scale > 1 ? info.Height : (int) Math.Floor(info.Height * scale);
					}
