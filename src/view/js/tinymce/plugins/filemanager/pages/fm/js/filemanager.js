	...
	window.FileManager = $.extend(BaseManager, {
		...
		showPreview : function(fi) {
			if (fi && (!fi.custom || fi.custom.previewable) && fi.type != 'zip')
				$('#preview').attr('src', '../../stream/default.aspx?cmd=fm.streamFile&path=' + encodeURIComponent(fi.path)).css('visibility', 'visible');
			else
		...
		downloadFile : function(f) {
			if (f.type != 'folder' && f.type != 'parent')
				$('#preview').attr('src', '../../stream/default.aspx?cmd=fm.download&path=' + encodeURIComponent(f.path));
		},

		viewFile : function(f) {
			if (f.type != 'folder' && f.type != 'parent' && f.type != 'zip')
				window.open('../../stream/default.aspx?cmd=fm.streamFile&path=' + encodeURIComponent(f.path), 'View'); 
		},
